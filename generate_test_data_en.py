...
def generate_test_data():
    # 球队列表（英文）
    teams = ["Barcelona", "RealMadrid", "ManchesterUnited", "Liverpool", "BayernMunich"]
    ...
    for file in os.listdir(meta_dir):
        os.remove(os.path.join(meta_dir, file))
        ...
        for category in categories:
            for year in years:
                ...
                for i in range(2):  # 每个款式生成2张图片
                    ...
                    # 保存图片
                    image_name = f"{team}_{year}_{category}_{i}.jpg"
                    ...
                    # 保存元数据
                    meta_file = os.path.join(meta_dir, f"{team}_{year}_{category}.json")
